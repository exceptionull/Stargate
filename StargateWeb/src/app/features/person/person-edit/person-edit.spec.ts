...
describe('PersonEdit', () => {
  ...
  describe('on edit', () => {
    ...
    it('should set formSubmitted to true and display error toast when api call returns error', () => {
        ...
        const personPutSpy = vi.spyOn(personService, 'personPut').mockReturnValue(throwError(() => error) as any)

        component.edit();

        expect(personPutSpy).toHaveBeenCalledExactlyOnceWith(formValue);
        expect(component.formSubmitted).toBe(true);
    ...
    it('should display success toast, reset form, set formSubmitted to false, emit success, and set visible to false when api call returns success', () => {
        ...
        const formResetSpy = vi.spyOn(component.form, 'reset');
        const personPutSpy = vi.spyOn(personService, 'personPut').mockReturnValue(of({ id: 1 }) as any)

        component.edit();

        expect(personPutSpy).toHaveBeenCalledExactlyOnceWith(formValue);
        expect(messageServiceAddSpy).toHaveBeenCalledExactlyOnceWith({ severity: 'success', summary: 'Success', detail: 'Person updated.' });
        expect(formResetSpy).toHaveBeenCalledOnce();
        expect(component.formSubmitted).toBe(false);

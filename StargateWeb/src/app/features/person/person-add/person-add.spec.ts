...
describe('PersonAdd', () => {
  ...
  describe('on add', () => {
    ...
    it('should set formSubmitted to true and display error toast when api call returns error', () => {
        ...
        const personPostSpy = vi.spyOn(personService, 'personPost').mockReturnValue(throwError(() => error) as any)

        component.add();
        
        expect(personPostSpy).toHaveBeenCalledExactlyOnceWith(formValue);
        expect(component.formSubmitted).toBe(true);
    ...
    it('should display success toast, reset form, set formSubmitted to false, emit success, and set visible to false when api call returns success', () => {
        ...
        const formResetSpy = vi.spyOn(component.form, 'reset');
        const personPostSpy = vi.spyOn(personService, 'personPost').mockReturnValue(of({ id: 1 }) as any)

        component.add();

        expect(personPostSpy).toHaveBeenCalledExactlyOnceWith(formValue);
        expect(messageServiceAddSpy).toHaveBeenCalledExactlyOnceWith({ severity: 'success', summary: 'Success', detail: 'Person added.' });
        expect(formResetSpy).toHaveBeenCalledOnce();
        expect(component.formSubmitted).toBe(false);
